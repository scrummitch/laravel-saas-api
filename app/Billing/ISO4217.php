...
class ISO4217
{
    public static function make(string|Currency|\Money\Currency $name)
    {
        if ($name instanceof Currency) {
            return $name;
        }

        if ($name instanceof \Money\Currency) {
            $name = $name->getCode();
        }

        return (new self)->findByCode($name);
    }
    ...
    public function findByCode($code)
    {
        ...
        foreach (static::$rawCurrencies as $currency) {
            if (strcasecmp($code, $currency['alpha3']) === 0) {
    ...
    public static function forCountryCode(string $country)
    {
        return (new self)->findByCountryCode($country);
    }
    ...
    public function findByNumeric($numeric)
    {
        ...
        foreach (static::$rawCurrencies as $currency) {
            if (strcasecmp($numeric, $currency['numeric']) === 0) {
                return $this->create($currency);
            }
        }
    ...
    {
        if (! isset($this->currencies[$currency['alpha3']])) {
            $this->currencies[$currency['alpha3']] = new Currency(
                $currency['name'],
                $currency['alpha3'],
                intval($currency['numeric']),
                intval($currency['exp']),
                $currency['country']
            );
        }
     ...
     * @var array
     */
    protected static $rawCurrencies = [
        [
            'name' => 'UAE Dirham',
            'alpha3' => 'AED',
            'numeric' => '784',
    ...
    private function findByCountryCode(string $country)
    {
        foreach (self::$rawCurrencies as $rawCurrency) {
            if (is_array($rawCurrency['country']) && in_array($country, $rawCurrency['country'])) {
                return self::create($rawCurrency);
            }

            if ($rawCurrency['country'] === $country) {
                return self::create($rawCurrency);
            }
        }
